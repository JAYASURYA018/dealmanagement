...
export class SalesforceApiService {
    ...
    getQuotePreview(quoteId: string): Observable<any> {
        ...
        const query = `SELECT Id, Name, QuoteNumber, Status, GrandTotal, StartDate, ExpirationDate, Pricebook2Id, Opportunity.Name, Opportunity.Sales_Channel__c, Opportunity.Primary_Contact__c, Opportunity.Primary_Contact__r.Name, Account.Name, Account.Website, (SELECT Id, Product2Id, PricebookEntryId, Product2.Name, Product2.ProductCode, Product2.Family, Product2.Description, Quantity, UnitPrice, TotalPrice, ListPrice, StartDate, EndDate, Discount, Incentive__c, NetUnitPrice FROM QuoteLineItems) FROM Quote WHERE Id='${quoteId}'`;
        const encodedQuery = encodeURIComponent(query);
