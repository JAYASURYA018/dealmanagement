...
export class SalesforceApiService {
     ...
     * Creates a Quote with Quote Lines using the Salesforce Composite Graph API
     */
    createQuoteWithLines_REMOVE(opportunityId: string, pricebookId: string, items: any[]): Observable<any> {
        const method = 'SalesforceApiService.createQuoteWithLines';
        const token = this.contextService.accessToken;
        const baseUrl = this.contextService.apiBaseUrl || 'https://vector--rcaagivant.sandbox.my.salesforce.com';
        const url = `${baseUrl}/services/data/v65.0/connect/rev/sales-transaction/actions/place`;

        // 1. Fetch PricebookEntries for all items
        const productIds = items.map(item => item.id);

        return this.getPricebookEntries(productIds).pipe(
            switchMap((pbeResponse: any) => {
                const pbeRecords = pbeResponse.records || [];
                console.log(`[API Response] ${method} (PricebookEntries)`, pbeRecords);

                // Use the Pricebook2Id from the first record if available, else fallback to passed pricebookId
                const dynamicPricebookId = pbeRecords.length > 0 ? pbeRecords[0].Pricebook2Id : pricebookId;
                const quoteName = `SubscriptionQuoteApi-${new Date().getTime()}`;
                ...
                // Add dynamic quote lines
                items.forEach((item, index) => {
                    // Find matching PricebookEntry for this product
                    const matchingPBE = pbeRecords.find((pbe: any) => pbe.Product2Id === item.id);
                    const finalPBEId = matchingPBE ? matchingPBE.Id : (item.pricebookEntryId || item.defaultPrice?.pricebookEntryId || '01uDz00000dqLY8IAM'); // Fallback Needs to be real

                    // Default values based on user request if not present in item
                    const subscriptionTerm = item.subscriptionTerm || 12;
                    const subscriptionTermUnit = item.subscriptionTermUnit || 'Monthly'; // Request says Monthly implies counts? Or Unit=Month? API expects 'Month' usually. User payload said "Monthly" in Unit? Salesforce Picklist value is usually 'Month'. Let's trust user payload or standard? User JSON: "SubscriptionTermUnit": "Monthly". Standard is 'Month'. I will use item value or 'Month'.
                    const billingFrequency = item.billingFrequency || 'Annual';
                    ...
                    records.push({
                        ...
                        "record": {
                            ...
                            "PeriodBoundary": item.periodBoundary || "Anniversary",
                ...
                console.log(`[API Request] ${method}`, { url, body: JSON.stringify(body, null, 2) });
